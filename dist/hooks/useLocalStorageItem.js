...
function useLocalStorageItem(keyName, defaultValue, encode, decode) {
  ...
  var _d = React.useState(true),
    available = _d[0],
    setAvailable = _d[1];
  var _e = React.useState(new Date().getTime()),
    renderTime = _e[0],
    setRenderTime = _e[1];
  React.useEffect(
    function () {
  ...
  React.useEffect(function () {
    if (!loading && available && !shouldPush) {
      ...
      var trueValue = localStorage.getItem(keyName);
  ...
  React.useEffect(function () {
    ...
    emitter_singleton_1
      .getEmitterSingleton()
      .on(emitter_singleton_1.clearEvent, clearListener);
    return function () {
      emitter_singleton_1
        .getEmitterSingleton()
        .off(keyName, itemChangeListener);
      emitter_singleton_1
        .getEmitterSingleton()
        .off(emitter_singleton_1.clearEvent, clearListener);
      aborted = true;
    };
  });
