 ...
 * and [`JSON.parse()`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/JSON/parse) for string encoding, so make sure your object is compatible with that interface. For objects that `JSON.stringify()` can't handle, provide your own encoding via [useLocalStorageItem](https://justinmahar.github.io/react-use-window-localstorage/useLocalStorageItem).
 *
 ...
 */
export function useLocalStorageObject(keyName: string, defaultValue: any | null = null): LocalStorageItem<any> {
